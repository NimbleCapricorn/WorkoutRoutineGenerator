...
class INOL_Target(Enum): #TODO# this should be configurable as well [but these are scientific limits, so they should not be too far off for anyone]
    ...
    DailyRecoverable=0.6    #between 0.4 and 1.0 is recoverable stimulus (next day you can repeat the same workout basically)
    LoadAccumulating=1.0    #1.0 to 2.0 is accumulating fatigue, with bigger strength gains (only once you fully recover, though)
